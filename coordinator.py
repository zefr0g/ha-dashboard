...
class ProxmoxQEMUCoordinator(ProxmoxCoordinator):
    ...
    async def _async_update_data(self) -> ProxmoxVMData:
        ...
        # Try to get accurate disk usage from QEMU guest agent
        ...
        try:
            ...
            if isinstance(entries, list):
                ...
                for fs in entries:
                    ...
                    total = fs.get("total-bytes-privileged") or fs.get("total-bytes")
                    if not device_key or not isinstance(used, (int, float)):
                        continue

                    used_by_device[device_key] = max(
                        used_by_device.get(device_key, 0),
                        int(used),
                    )
                    if isinstance(total, (int, float)):
                        total_by_device[device_key] = max(
                            total_by_device.get(device_key, 0),
                            int(total),
                        )

                if used_by_device:
                    guest_disk_used = sum(used_by_device.values())
                if total_by_device:
                    guest_disk_total = sum(total_by_device.values())
